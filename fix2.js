...
const returnIndex = newStr.lastIndexOf('  return (');
const funcBody = newStr.slice(funcStart, returnIndex);

const adminWrapperStart = origStr.indexOf('<div className="admin-wrapper"');
const origReturnStart = origStr.lastIndexOf('  return (', adminWrapperStart);
...
const origFormEnd = origStr.indexOf('<div style={{ marginTop: 32, display: "flex", justifyContent: "flex-end" }}>', origFormStart);
const layoutFooter = origStr.slice(origFormEnd);
...
const finalFile = head + funcBody + layoutHeader + formJSX + layoutFooter + tail;

fs.writeFileSync('src/components/AgentSkillsManager.tsx', finalFile, 'utf8');
console.log('Rebuilt successfully! Form length:', formJSX.length, 'Footer length:', layoutFooter.length);
