...
const buf = fs.readFileSync('src/components/AgentSkillsManager.tsx');
...
const old_interface_start = content.indexOf('interface SkillFormData {');
const old_interface_end = content.indexOf('export function AgentSkillsManager');

const new_interface = `interface SkillFormData {
  name: string;
  description: string;
  color: string;
...
content = content.substring(0, old_interface_start) + new_interface + content.substring(old_interface_end);

// 3. Replace loadSkillForm and handleSubmit
const old_func_start = content.indexOf('  const loadSkillForm = (agent: ValorantAgent, key: string) => {');
const old_func_end = content.indexOf('  const handleFetchIcon = async () => {', old_func_start);

const new_funcs = `  const loadSkillForm = (agent: ValorantAgent, key: string) => {
...
content = content.substring(0, old_func_start) + new_funcs + content.substring(old_func_end);

// 4. Replace form tabs and content
const form_tabs_start = content.indexOf('                  <div style={{ display: "flex", gap: 12, marginBottom: 24, borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: 16 }}>');
const form_tabs_end = content.indexOf('                  <div style={{ marginTop: 32, display: "flex", justifyContent: "flex-end" }}>', form_tabs_start);

const new_form_content = `                  <div style={{ display: "flex", gap: 12, marginBottom: 24, borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: 16 }}>
...
content = content.substring(0, form_tabs_start) + new_form_content + content.substring(form_tabs_end);
