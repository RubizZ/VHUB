...
function buildGeometry(
    ...
    switch (type) {
        case "circle":
            return { type: "circle", radius: radius || undefined };
        case "cone":
            return {
                type: "cone",
...
function buildDeployment(f: SkillFormData): DeploymentMechanics {
    ...
    switch (f.dep_type) {
        ...
        case "projectile_terminal_aoe":
            return {
                type: "projectile_terminal_aoe",
                windup,
                projectileSpeed: f.dep_projectileSpeed || undefined,
                projectileMaxDistance: f.dep_projectileMaxDistance || undefined,
                bounces: f.dep_infiniteBounces ? undefined : (f.dep_bounces || undefined),
                infiniteBounces: f.dep_infiniteBounces || undefined,
        ...
        case "equip_weapon":
            return {
                type: "equip_weapon",
                windup,
                maxAmmo: f.dep_maxAmmo || undefined,
            };
        default:
            return { type: "self_instant", windup };
    }
...
export function AgentSkillsManager({
    ...
    const [selectedAgent, setSelectedAgent] = useState<ValorantAgent | null>(
        null,
    );
    const [editingSkillKey, setEditingSkillKey] = useState<string>("q");
    const [activeTab, setActiveTab] = useState<
        "general" | "deployment" | "lifetime" | "resolution"
    >("general");
    const [formData, setFormData] =
        useState<SkillFormData>(getDefaultFormData());
    ...
    const loadSkillForm = (agent: ValorantAgent, key: string) => {
        ...
        const readGeom = (
            ...
            | "dep_geom_width"
            | "dep_geom_length"
        > & { gType: SkillFormData["dep_geom_type"] } => {
            if (!geom)
                return {
                    gType: "none",
                    dep_geom_type: "none",
            ...
            const angle =
                "angle" in geom ? (geom as { angle?: number }).angle || 0 : 0;
            const width =
                "width" in geom ? (geom as { width?: number }).width || 0 : 0;
            const length =
                "length" in geom
                    ? (geom as { length?: number }).length || 0
                    : 0;
            return {
                gType,
                dep_geom_type: gType,
                dep_geom_radius: radius,
                dep_geom_angle: angle,
                dep_geom_width: width,
                dep_geom_length: length,
            };
        };

        const depGeom =
            "geometry" in d
                ? readGeom((d as { geometry?: SkillGeometry }).geometry)
                : readGeom(undefined);
        ...
        setFormData({
            ...
            rechargeType: skill.economy?.rechargeCondition?.type || "kills",
            rechargeValue: skill.economy?.rechargeCondition?.value || 2,

            dep_type:
                ("type" in d
                    ? (d as { type: DeploymentType }).type
                    : "self_instant") || "self_instant",
            dep_windup:
                ("windup" in d
                    ? (d as { windup?: number }).windup
                    : undefined) || 0,
            dep_spawnOffset:
                ("spawnOffset" in d
                    ? (d as { spawnOffset?: number }).spawnOffset
                    : undefined) || 0,
            dep_castRange:
                ("castRange" in d
                    ? (d as { castRange?: number }).castRange
                    : undefined) || 0,
            dep_projectileSpeed:
                ("projectileSpeed" in d
                    ? (d as { projectileSpeed?: number }).projectileSpeed
                    : undefined) || 0,
            dep_projectileMaxDistance:
                ("projectileMaxDistance" in d
                    ? (d as { projectileMaxDistance?: number })
                          .projectileMaxDistance
                    : undefined) || 0,
            dep_bounces:
                ("bounces" in d
                    ? (d as { bounces?: number }).bounces
                    : undefined) || 0,
            dep_infiniteBounces:
                ("infiniteBounces" in d
                    ? (d as { infiniteBounces?: boolean }).infiniteBounces
                    : undefined) || false,
            dep_steerable:
                ("steerable" in d
                    ? (d as { steerable?: boolean }).steerable
                    : undefined) || false,
            dep_traversesWalls:
                ("traversesWalls" in d
                    ? (d as { traversesWalls?: boolean }).traversesWalls
                    : undefined) || false,
            dep_variableDistance:
                ("variableDistance" in d
                    ? (d as { variableDistance?: boolean }).variableDistance
                    : undefined) || false,
            dep_maxAmmo:
                ("maxAmmo" in d
                    ? (d as { maxAmmo?: number }).maxAmmo
                    : undefined) || 0,
            dep_directionalOnly:
                ("directionalOnly" in d
                    ? (d as { directionalOnly?: boolean }).directionalOnly
                    : undefined) || false,
            ...
            hasLifetime:
                skill.lifetime != null &&
                Object.keys(skill.lifetime).length > 0,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const economy: EconomyData = {
            ...
            rechargeCondition: formData.hasRecharge
                ? { type: formData.rechargeType, value: formData.rechargeValue }
                : undefined,
        };
            ...
            ? {
                  ...
                  autonomous:
                      formData.life_behavior === "autonomous"
                          ? {
                                activationRadius:
                                    formData.life_activationRadius || undefined,
                                movementSpeed:
                                    formData.life_movementSpeed || undefined,
                            }
                          : undefined,
                  geometry: buildGeometry(
                      formData.life_geom_type,
                      formData.life_geom_radius,
                      formData.life_geom_angle,
                      formData.life_geom_width,
                      formData.life_geom_length,
                  ),
              }
            : null;
        ...
        const resolution: ResolutionMechanics | null = formData.hasResolution
            ? {
                  geometry: buildGeometry(
                      ...
                      formData.res_geom_radius,
                      formData.res_geom_angle,
                      formData.res_geom_width,
                      formData.res_geom_length,
                  ),
              }
            : null;
    ...
    const handleFetchIcon = async () => {
        ...
        try {
            const res = await fetch(
                `https://valorant-api.com/v1/agents/${selectedAgent.id}?language=es-ES`,
            );
            ...
            if (slot) {
                const ability = abilities.find(
                    (a: { slot: string; displayIcon: string }) =>
                        a.slot === slot,
                );
    ...
    const GeomEditor = ({
        ...
        <div>
            <select
                ...
                value={geomType}
                onChange={(e) =>
                    onType(e.target.value as SkillFormData["dep_geom_type"])
                }
    ...
    return (
        ...
        >
                ...
                <>
                    ...
                    >
                        <div>
                            ...
                            >
                                Configuración de Despliegue, Vida y Resolución
                            </p>
                        </div>
                        <div>
                    ...
                    {isLoading ? (
                        ...
                        <div className="grid grid-4" style={{ gap: 24 }}>
                            {filteredAgents.map((agent) => (
                                <div
                                    key={agent.id}
                                    className="agent-card card glass-card"
                                ...
                                >
                                    ...
                                    <div
                                        className="agent-skills-preview"
            ...
            {selectedAgent && (
                <div
                    className={isModalMode ? "" : "modal-overlay"}
                    style={isModalMode ? { padding: 20 } : {}}
                >
                    <div
                        className={
                            isModalMode
                                ? ""
                                : "modal-content card glass-card premium-modal"
                        }
                        style={
                            isModalMode
                                ? {
                    ...
                    >
                        {!isModalMode && (
                            ...
                            >
                                <div>
                                    ...
                                    >
                                        Habilidades de {selectedAgent.name}
                                    </h2>
                                    <p
                                        style={{
                                            color: "var(--text-muted)",
                        ...
                        <div style={{ display: "flex", gap: 24 }}>
                            ...
                            <div
                                style={{
                                    flex: 1,
                                    maxHeight: isModalMode ? "none" : "70vh",
                                    overflowY: "auto",
                                    paddingRight: 8,
                                }}
                            >
                                        ...
                                        <label
                                            ...
                                            style={{
                                                ...
                                                background: formData.enabled
                                                    ...
                                                    : "rgba(255, 70, 85, 0.1)",
                                                borderRadius: 8,
                                                border: `1px solid ${formData.enabled ? "rgba(0, 212, 170, 0.3)" : "rgba(255, 70, 85, 0.3)"}`,
                                            }}
                                        >
                                            <input
                                                type="checkbox"
                                        ...
                                        <div className="tab-content fade-in">
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    <input
                                                        className="input-field"
                                                        placeholder="ej. Usa estrellas compartidas"
                                                        value={
                                                            formData.costNote
                                                        }
                                                        onChange={(e) =>
                                    ...
                                    {activeTab === "deployment" && (
                                        <div className="tab-content fade-in">
                                            ...
                                            >
                                                ...
                                                >
                                                    <option value="self_instant">
                                                        Instante (Bufo Propio)
                                                    </option>
                                                    <option value="dash_teleport">
                                                        Movimiento (Dash /
                                                        Teletransporte)
                                                    </option>
                                                    <option value="projectile_terminal_aoe">
                                                        Proyectil → Área
                                                        Terminal (Detonación)
                                                    </option>
                                                    <option value="projectile_sweeping">
                                                        Proyectil → Barrido en
                                                        Vuelo (Sweeping)
                                                    </option>
                                                    <option value="map_target_aoe">
                                                        Target en Mapa Táctico →
                                                        Área
                                                    </option>
                                                    <option value="linear_wall">
                                                        Muro Lineal
                                                    </option>
                                                    <option value="two_point_barrier">
                                                        Barrera de 2 Puntos
                                                    </option>
                                                    <option value="self_mobile_aura">
                                                        Aura Móvil Personal
                                                    </option>
                                                    <option value="static_deployable">
                                                        Desplegable Estático
                                                        (Trampa/Torreta)
                                                    </option>
                                                    <option value="equip_weapon">
                                                        Equipar Arma
                                                    </option>
                                                </select>
                                            </div>
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            ...
                                                            <input
                                                                ...
                                                                className="input-field"
                                                                value={
                                                                    formData.dep_projectileMaxDistance
                                                                }
                                                                onChange={(e) =>
                                                                    setFormData(
                                                                        {
                                                        ...
                                                        >
                                                            ...
                                                            >
                                                                ...
                                                                <input
                                                                    ...
                                                                    className="input-field"
                                                                    value={
                                                                        formData.dep_bounces
                                                                    }
                                                                    disabled={formData.dep_infiniteBounces}
                                                                    onChange={(
                                                                        e,
                                                                    ) =>
                                                                        setFormData(
                                            ...
                                            {formData.dep_type ===
                                                ...
                                                >
                                                    ...
                                                    >
                                                        Geometría del Barrido
                                                    </h4>
                                                    <GeomEditor
                                                        geomType={
                                                            formData.dep_geom_type
                                                        }
                                                        radius={
                                                            formData.dep_geom_radius
                                                        }
                                                        angle={
                                                            formData.dep_geom_angle
                                                        }
                                                        width={
                                                            formData.dep_geom_width
                                                        }
                                                        length={
                                                            formData.dep_geom_length
                                                        }
                                                        onType={(v) =>
                                                            setFormData({
                                                                ...formData,
                                                            ...
                                                            >
                                                                ...
                                                                >
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        className="input-field"
                                                                        value={
                                                                            formData.life_activationRadius
                                                                        }
                                                                        onChange={(
                                                                            e,
                                                                        ) =>
