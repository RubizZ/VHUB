...
export function BottomNav() {
  ...
  const baseLinks = [
    ...
    { href: "/strategies", label: "Estrategias", icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polygon points="1 6 1 22 8 18 16 22 23 18 23 2 16 6 8 2 1 6"></polygon><line x1="8" y1="2" x2="8" y2="18"></line><line x1="16" y1="6" x2="16" y2="22"></line></svg>
    ) },
    { href: "/matches", label: "Partidos", icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M6 12L10 16L18 8"/><circle cx="12" cy="12" r="10"/></svg>
    ) },
    ...
    { href: "/leaderboard", label: "Clasificación", icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 2H6v7a6 6 0 0 0 12 0V2Z"/></svg>
    ) },
  ];

  const teamLinks = [
    { href: "/team/roster", label: "Plantilla", icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
    ) },
    { href: "/team/settings", label: "Ajustes de Equipo", icon: (
  ...
  const adminLinks = [
    { href: "/admin", label: "Panel Global", icon: (
    ...
    { href: "/admin/teams", label: "Equipos", icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="7" width="20" height="14" rx="2" ry="2"/></svg>
    ) },
  ...
  const profileLinks = [
    ...
    { href: "/settings", label: "Cuenta", icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>
    ) },
  ];
  ...
  let allLinks = [
    ...baseLinks,
    ...(role === 'team_admin' || role === 'super_admin' ? teamLinks : []),
    ...(role === 'super_admin' ? adminLinks : []),
    ...profileLinks,
  ];

  // remove duplicates preserving first occurrence (dedupe by href)
  ...
  // Define essential links to show directly in the bottom nav (4 links + Menu = 5 total)
  const essentialHrefs = ["/", "/chat", "/availability", "/matches"];
  const essentialLinks = allLinks.filter(l => essentialHrefs.includes(l.href));
  const otherLinks = allLinks.filter(l => !essentialHrefs.includes(l.href));
  ...
  return (
    <>
      ...
      <MobileMenu isOpen={isMenuOpen} onClose={() => setIsMenuOpen(false)} links={otherLinks} />
    </>
  );
}
